...
def reporte_a_Excel():
    while True:
        fecha_reporte = input("¿De qué fecha quieres sacar el reporte? ")
        fecha_reporte = datetime.strptime(fecha_reporte, "%d/%m/%Y")
        print("*" * 74)
        print(f"**            REPORTE DE RESERVACIONES PARA EL DIA {fecha_reporte.strftime('%d/%m/%Y')}           **")
        print("*" * 74)
        try:
            with sqlite3.connect("34.db", detect_types=sqlite3.PARSE_DECLTYPES | sqlite3.PARSE_COLNAMES) as conn:
                mi_cursor = conn.cursor()
                ...
                if registrados:
                    ...
                    for folio, nombre, horario, fecha in registrados:
                        print("Clave\t" + "Nombre\t" + " Turno\t" + "            Fecha\t")
                        print(f"{folio}\t{nombre}\t{horario}\t{fecha.strftime('%d/%m/%Y')}")
                        item = (folio, nombre, horario, fecha)  # Utilizar una tupla para cada registro
                        lista.append(item)

                    # Crear el archivo Excel fuera del bucle for
                    wb = openpyxl.Workbook()
                    hoja = wb.active

                    for i, column_name in enumerate(["Clave", "Nombre", "Turno", "Fecha"], start=1):
                        hoja.cell(row=1, column=i, value=column_name)

                    for row, item in enumerate(lista, start=2):
                        for col, value in enumerate(item, start=1):
                            hoja.cell(row=row, column=col, value=value)

                    wb.save('reservaciones.xlsx')
                    print(f'Datos exportados a reservaciones.xlsx')
                else:
        ...
        volver_al_menu = input("¿Deseas volver al menú? (Sí/No): ")
        if volver_al_menu.upper() != 'SÍ':
            return
